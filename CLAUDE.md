...
- 涉及檔案：`global.css`＋process 頁＋`favicon.svg`；**`public/process/` 實際是九個 `.html`，每一個都有自己的 `--a`**（本節舊稱「七頁」，2026-07-28 實測與目錄對不上，該以 `ls public/process/` 為準）；收工前 `grep -ri "<舊色碼>" src public` 必須零殘留
...
**`public/process/` 明文豁免：那裡的主色文字是刻意保留的歷史狀態，不是漏網的違規。** 那些頁的說話者標籤、選項標籤、表頭、數字都用主色、而且全部不可點，用的又是真變數 `--a`（不是展示用的凍結色碼），所以掃描一定會掃到——掃到就跳過，不要再為此開一輪討論。理由：換色協議授權的是「同一個角色換一個色碼」，**不授權「改變誰扮演哪個角色」**，後者會讓歷史存檔不再是當時的樣子。

可枚舉，不必憑印象：`grep -nE "(^|[^-])color: ?var\(--a[,)]" public/process/*.html` — 2026-07-28 實測**九個檔共 39 處**（其中九處是 `a` 連結、四處是示範按鈕）。舊敘述的「七頁 41 處」是把兩處 `border-color: var(--a)` 一起數了（39＋2），而框線是非文字通道、本規則不管它——以上面那條命令為準。

**兩件刻意不修的，登記在案，不必再當成新發現：**

- **`.cs-wipe-tag--new` 的邊框沒有任何閘門在看**——對比檢查只驗文字色，不驗 `border-color`。今天安全是截圖角落顏色的巧合：實測某張截圖的金色角落對主色框只有 **1.322**，只因落在另一側才沒事。
