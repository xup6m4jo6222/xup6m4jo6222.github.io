...
> **⚠ 這一節正在被改寫，下面那句「只允許當兩類文字」自 2026-07-29 起已經不完整。**
> 「元素主色的份量」那一輪加了第三類落點：**「線」元件上的標籤**（主色只以線的形式存在、
> 沒有鋪亮字底下那塊底時，標籤的字可以拿主色，辨識改由襯線 600 承擔）。票 04 已落地
> `.dmr th` 與 `.tl-solo em`，清單因此是**九條**不是七條。整節的改寫排在票 06（要等票 05
> 的「面」落地才動得了）。**在那之前，以 `ACCENT_TEXT_ALLOWLIST` 的內容為準，不要以這段
> 敘述為準**——閘門讀的是清單，不是這段話。

`src/` 裡主色只允許當兩類文字：**內文連結靜止態**，以及**靜止態已在中性階頂端（n-900）的互動元素其 hover 態**（外加把手字符 `.cs-wipe-handle:after`）。唯一的清單是 `scripts/palette-config.mjs` 的 `ACCENT_TEXT_ALLOWLIST`，共七條，每一條都帶著它的辨識通道與那個通道宣告在哪個選擇器（「連結」那組配對的 `fgOn` 自此由它推導，不要另外列一份）；不在清單上而把主色當文字色，`npm run verify:palette` 會紅。**這條規則的敘述一律不得寫「全站」**——它管不到 `public/`。
...
- **`--color-bg-alt`（決策開關、角標卡）封頂在 n-100 的理由 2026-07-29 回來了，而且比原本更硬。** 票 04 把角標卡的前導小標 `.tl-solo em` 改回主色（「線」的落點），所以那塊底再提亮就會壓到它自己的字：實測**主色字在 n-100 上 4.84（過），提到 n-200 剩 3.61（破線）**。要提階就得先把那個小標的顏色收回去——**兩件事現在是綁在一起的**，不再是「另一個決定」。
  （順帶：含顆粒的實際值 n-100 是 4.32、n-200 是 3.18，兩個都不到 4.5——顆粒缺口見 SPEC-accent-weight「Out of Scope」，那是另一輪。）
