...
- **閘門有一個顆粒缺口。** 顆粒材質（`body::before`，soft-light 17%）蓋著整個視窗，但閘門只對「頁面表面」那一組模擬顆粒，**元件表面不模**——理由見 `palette-config.mjs`：`body::before` 的 `z-index: -1` 讓它畫在內容之下，所以 `.card`／`.cs-track` 這些**有不透明底**的表面確實不吃顆粒。缺口在**半透明或元件底上的字**：角標卡小標 `.tl-solo em` 疊 n-100，閘門算 **4.84（過）**，含顆粒亮處實際是 **4.32（不過）**。影響面大於那一輪，SPEC-accent-weight「Out of Scope」明文另開一輪。**綠燈在這一格的意思是「沒有漂移」，不是「一定達標」。**
