...
const cases = [
	...
	{
		/* **上限**。抗辯第三輪實測：先前守衛只有下限，把**判準檔**的冷卻改到 7000
		   整條管線全綠——而那正是第二輪判定「敘事不可觀察」、第三輪宣稱修掉的
		   那個失敗態。執行期的節奏檢查也擋不住，因為它的期望值是拿產物自己的
		   rearm 算的，任何值都自我一致（rearm=60000 → 六秒一次都沒晃，
		   檢查照樣印「✓ 節奏對得上」）。
		...
		expectPass: false,
		mutate: (_f, dir) => injectCraft(dir, { ...CFG.FIELD_CRAFT, shockRearmMs: 7000 }),
		expect: /shockRearmMs（退化值）[\s\S]*實質不可觀察/,
	},
	{
