...
export const MOTIF_CRAFT = {
	 ...
	 * 已判定）：真正的依據是票 05 三點擬合出的每幀成本 **0.33ms ＋ 0.0021ms × 點數**，
	 * 11ms 預算下桌機擺得下約 5,075 個點。1870 點推得 ≈4.3ms，仍在預算的四成內。
	 *
	 * **兩筆未結的帳**：①那條擬合是無頭桌機量的，真機（低階與舊裝置）從未量過——票 07。
	...
	layout: {
		...
		/**
		 * 趨勢線最實處的不透明度。**票 04 的墨量三檔**：舊值 0.62。
		 * 拉滿的理由是那條線本來就在（`trendBand` 橫跨全寬、故事 2026-07-27 已登記），
		 * 只是實度不夠所以沒被畫出來——**這一票沒有動它的位置，只動它的實度**。
		 */
		trendAlpha: 1,
...
export const FIELD_CRAFT = {
	...
	alphaBuckets: 12,
	/**
	 * 記錄下來的裂縫數上限，**每百萬掃描像素幾個**（次級線與餘燼都長在它們身上）。
	 *
	 * 原本寫死 3000。抗辯實測：1280×720 只用到 239、超寬 3440×1440 用到 1305，
	 * 但 **5120×2880 需要 4292，於是在 x=3726（畫面 72.8%）觸頂**——掃描迴圈是
	 * x 由左而右，觸頂之後右邊 27% 完全收不到裂縫，那一帶沒有次級線也沒有餘燼，
	 * 是肉眼看得出的左右不對稱，四條敘事在那裡只剩兩條。閘門與探針都只在
	 * 1280／1920 跑，所以量不到。
	 *
	 * 改成密度之後每個尺寸都不再截斷，而它仍然是一道失控防線——**與「點密度用密度
	 * 不用絕對數量」同一個道理**：寫死數量會讓同一個設計在大螢幕上變成另一個東西。
	 * 3300 保住原本在 1280×720 的餘裕量級（實際用量的十倍出頭）。
	 */
	crackCapPerMpx: 3300,
	...
	/**
	 * 回彈的**冷卻時間**（ms）：距離上一次回彈超過這麼久，下一次捲動才會再觸發一次。
	 *
	 * 兩件事分開講。
	 *
	 * **為什麼需要冷卻**：真實捲動（滾輪與慣性）每一幀送一次 `scroll` 事件。沒有冷卻的話
	 * 每一次都把幅度重設回滿格，回彈就不是「觸發一次、350ms 回位」而是「捲多久晃多久」
	 * ——2026-07-29 抗辯實測，持續捲動期間逐幀畫面差 45716、靜置底噪 53，位移全程滿幅。
	 * 那同時踩掉動效紅線①與敘事本身（一直在震，何來恢復）。
	 *
	 * **為什麼從「上一次回彈」算而不是「上一次 scroll 事件」算**：後者是第一版的修法，
	 * 抗辯第二輪實測它會讓「事件間隔恆小於冷卻」的節奏永遠不武裝——滾輪每 400–600ms
	 * 一格時，整段閱讀只出現一次回彈，敘事等於不可觀察。
	 *
	 * **值怎麼來的**：四關夾得出下限夾不出這個數。理念一與第四關（注意力）都只說
	 * 「別太密」；**關三給得出硬下限**——敘事是「我恢復得很快」，回彈之後必須有一段
	 * 看得見的靜止，否則沒有「恢復」可言，所以冷卻必須明顯大於回位時間（下限取兩倍）。
	 * 中間那一段沒有判準，與線的強度、裂縫比例一樣**由本人看比對圖決定**：
	 * 2026-07-29 看過 0.7s／3s／8s 三個候選的節奏時間軸（12 秒連續捲動分別晃
	 * 17／4／2 次）之後選 2.7 秒。
	 */
	shockRearmMs: 2700,
	/**
	 * 「一次連續捲動」的量級（ms）——探針量節奏用的視窗，也是冷卻上限的來源。
	 * 上限 ＝ 這個視窗的一半：一次連續捲動裡回彈**至少要出現兩次**，否則敘事
	 * 「捲動觸發整場回彈」實質不可觀察（那正是抗辯第二輪抓到的失敗態）。
	 * 閘門與探針讀同一個值，不各寫一份。
	 */
	rhythmWindowMs: 6000,
