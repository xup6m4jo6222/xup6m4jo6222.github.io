...
export const FIELD_CRAFT = {
	 ...
	 * 回彈要**安靜這麼久才重新武裝**（ms）。
	 *
	 * 沒有這一條的話回彈不是「觸發一次、350ms 回位」，是「捲多久晃多久」：真實捲動
	 * （滾輪與慣性）每一幀送一次 scroll 事件，而每一次都把幅度重設回滿格。
	 * 2026-07-29 抗辯實測：持續捲動兩秒期間逐幀畫面差 45716，靜置底噪 53——差三個
	 * 數量級，位移全程維持滿幅。那讓 SPEC 寫的「單一動作 ≤350ms，上限就是紅線」
	 * 在真實捲動下不成立，也讓敘事「我恢復得很快」講不出來（一直在震，何來恢復）。
	 *
	 * 400 只要大於回位時間即可，本身無故事——**登記為技法**。
	 */
	shockRearmMs: 400,
