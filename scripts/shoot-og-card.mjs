...
<!-- 這一行必須跟 BaseLayout 的字體載入同步。**換字體時最容易漏掉的就是這裡**：
     卡片吃 global.css 的 --font-display，字沒載到不會報錯、只會靜靜落回系統字。
     2026-08-04 換成霞鶩文楷＋昭源圓體時就是這樣被抓到的。 -->
<link href="https://fonts.googleapis.com/css2?family=Chiron+GoRound+TC:wght@400;500&family=LXGW+WenKai+TC:wght@400;700&display=block" rel="stylesheet">
...
try {
	execFileSync(CHROME, [
		...
		`http://localhost:${port}/og-card-tmp.html`,
	], { stdio: 'ignore' });
} finally {
